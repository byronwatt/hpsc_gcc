...
enum riscv_ext_requirement_class_t
{
#define RISCV_EXT_REQ(REQ_NAME, REQ_CHECK, ERROR_MSG) REQ_NAME,
#include "riscv-extension-requirements.def"
  RV_NUM_EXT_REQUIREMENT,
  RV_NONE_EXT_REQUIREMENT = RV_NUM_EXT_REQUIREMENT
};

static const char *riscv_ext_req_err_msg[RV_NUM_EXT_REQUIREMENT] =
{
#define RISCV_EXT_REQ(REQ_NAME, REQ_CHECK, ERROR_MSG) ERROR_MSG,
#include "riscv-extension-requirements.def"
};

/* Type for ISA requirement info.  */
struct riscv_ext_requirement_t
{
  const char *ext;
  enum riscv_ext_requirement_class_t req_class;
};

static const riscv_ext_requirement_t riscv_ext_requirements[] =
{
  {"zvfh",      RV_REQUIRE_V_OR_ANY_ZVE},

  {"zvl32b",    RV_REQUIRE_V_OR_ANY_ZVE},
  {"zvl64b",    RV_REQUIRE_V_OR_ANY_ZVE},
  {"zvl128b",   RV_REQUIRE_V_OR_ANY_ZVE},
  {"zvl256b",   RV_REQUIRE_V_OR_ANY_ZVE},
  {"zvl512b",   RV_REQUIRE_V_OR_ANY_ZVE},
  {"zvl1024b",  RV_REQUIRE_V_OR_ANY_ZVE},
  {"zvl2048b",  RV_REQUIRE_V_OR_ANY_ZVE},
  {"zvl4096b",  RV_REQUIRE_V_OR_ANY_ZVE},
  {"zvl8192b",  RV_REQUIRE_V_OR_ANY_ZVE},
  {"zvl16384b", RV_REQUIRE_V_OR_ANY_ZVE},
  {"zvl32768b", RV_REQUIRE_V_OR_ANY_ZVE},
  {"zvl65536b", RV_REQUIRE_V_OR_ANY_ZVE},

  {"zvkns",  RV_REQUIRE_V_OR_ANY_ZVE_AND_ZVL128},
  {"zvknf",  RV_REQUIRE_V_OR_ANY_ZVE_AND_ZVL128},
  {"zvknha", RV_REQUIRE_V_OR_ANY_ZVE_AND_ZVL128},
  {"zvknhb", RV_REQUIRE_V_OR_ANY_ZVE_AND_ZVL128},
  {"zvkb",   RV_REQUIRE_V_OR_ANY_ZVE_AND_ZVL128},
  {"zvkg",   RV_REQUIRE_V_OR_ANY_ZVE_AND_ZVL128},
  {"zvksed", RV_REQUIRE_V_OR_ANY_ZVE_AND_ZVL128},
  {"zvksh",  RV_REQUIRE_V_OR_ANY_ZVE_AND_ZVL128},

  {NULL, RV_NONE_EXT_REQUIREMENT}
};
...
static const struct riscv_ext_version riscv_ext_version_table[] =
{
  ...
  {"zihintntl", ISA_SPEC_CLASS_NONE, 0, 2},
  ...
  {"xsfvqmaccdod", ISA_SPEC_CLASS_NONE, 0, 1},
  {"xsfvqmaccqoq", ISA_SPEC_CLASS_NONE, 0, 1},
  {"xsfvfwmaccqqq", ISA_SPEC_CLASS_NONE, 0, 1},
  {"xsfvfnrclipxfqf", ISA_SPEC_CLASS_NONE, 0, 1},
  {"xsfvcp", ISA_SPEC_CLASS_NONE, 0, 1},
  ...
  {"zvkns",  ISA_SPEC_CLASS_NONE, 0, 1},
  {"zvknf",  ISA_SPEC_CLASS_NONE, 0, 1},
  {"zvknha", ISA_SPEC_CLASS_NONE, 0, 1},
  {"zvknhb", ISA_SPEC_CLASS_NONE, 0, 1},
  {"zvkb",   ISA_SPEC_CLASS_NONE, 0, 1},
  {"zvkg",   ISA_SPEC_CLASS_NONE, 0, 1},
  {"zvksed", ISA_SPEC_CLASS_NONE, 0, 1},
  {"zvksh",  ISA_SPEC_CLASS_NONE, 0, 1},
  ...
  {"sscofpmf", ISA_SPEC_CLASS_NONE, 0, 1},
...
riscv_subset_list::parse_multiletter_ext (const char *p)
{
  ...
  unsigned minor_version = 0;
  int i;
  bool match_any_ext_type = false;
  const char *ext_types[] = {"s", "z", "x"};
  const char *ext_type_strs[] = {
    "supervisor extension",
    "sub-extension",
    "non-standard extension"
  };
  const char *ext_type = NULL;
  const char *ext_type_str = NULL;
  size_t max_ext_types = ARRAY_SIZE (ext_types);
  ...
  while (*p)
      ...
      if (*p == '_')
	{
      ...
      match_any_ext_type = false;
      for (int i = 0; i < max_ext_types; ++i)
	{
	  ext_type = ext_types[i];
	  ext_type_str = ext_type_strs[i];
	  size_t len = strlen (ext_types[i]);
	  if (strncmp (p, ext_types[i], len) == 0)
	    {
	      match_any_ext_type = true;
	      break;
	    }
	}

      if (!match_any_ext_type)
	break;
...
bool
riscv_subset_list::check_extension_requirements ()
{
  bool req_check_vector[RV_NUM_EXT_REQUIREMENT];
  const riscv_ext_requirement_t *ext_req_info;
  int i;
  riscv_subset_t *itr;

#define RISCV_CHECK_EXT(EXT_NAME) lookup(EXT_NAME)
#define RISCV_EXT_REQ(REQ_NAME, REQ_CHECK, ERROR_MSG) \
  req_check_vector[REQ_NAME] = REQ_CHECK;
#include "riscv-extension-requirements.def"

  for (itr = this->m_head; itr != NULL; itr = itr->next)
    for (ext_req_info = &riscv_ext_requirements[0];
	 ext_req_info->ext;
	 ++ext_req_info)
      {
	if (ext_req_info->ext != itr->name)
	  continue;

	if (!req_check_vector[ext_req_info->req_class])
	  {
	    error_at (
	      m_loc,
	      "%<-march=%s%>: Extension `%s' %s.",
	      m_arch, itr->name.c_str (),
	      riscv_ext_req_err_msg [ext_req_info->req_class]);
	    return false;
	  }
      }
...
riscv_subset_list::parse (const char *arch, location_t loc)
{
  ...
  p = subset_list->parse_multiletter_ext (p);
  ...
  do {
    ...
    for (itr = subset_list->m_head; itr != NULL; itr = itr->next)
      any_update |= subset_list->handle_implied_ext (itr);
  } while (any_update);

  if (!subset_list->check_extension_requirements())
    goto fail;
...
static const riscv_ext_flag_table_t riscv_ext_flag_table[] =
{
  ...
  {"zvkns",  &gcc_options::x_riscv_zv_subext, MASK_ZVKNS},
  {"zvknf",  &gcc_options::x_riscv_zv_subext, MASK_ZVKNF},
  {"zvknha", &gcc_options::x_riscv_zv_subext, MASK_ZVKNHA},
  {"zvknhb", &gcc_options::x_riscv_zv_subext, MASK_ZVKNHB},
  {"zvkb",   &gcc_options::x_riscv_zv_subext, MASK_ZVKB},
  {"zvkg",   &gcc_options::x_riscv_zv_subext, MASK_ZVKG},
  {"zvksed", &gcc_options::x_riscv_zv_subext, MASK_ZVKSED},
  {"zvksh",  &gcc_options::x_riscv_zv_subext, MASK_ZVKSH},

  {"zkt",       &gcc_options::x_riscv_zi_subext, MASK_ZKT},

  {"zicboz", &gcc_options::x_riscv_zicmo_subext, MASK_ZICBOZ},
  {"zicbom", &gcc_options::x_riscv_zicmo_subext, MASK_ZICBOM},
  {"zicbop", &gcc_options::x_riscv_zicmo_subext, MASK_ZICBOP},

  {"za64rs",   &gcc_options::x_riscv_zi_subext, MASK_ZA64RS},
  ...
  {"zic64b",   &gcc_options::x_riscv_zi_subext, MASK_ZIC64B},

  {"svinval", &gcc_options::x_riscv_sv_subext, MASK_SVINVAL},
  {"svnapot", &gcc_options::x_riscv_sv_subext, MASK_SVNAPOT},

  {"sscofpmf", &gcc_options::x_riscv_sv_subext, MASK_SSCOFPMF},
